// Insert Photo Italy blog posts from a batch file into the Italian database.
//
// The batch files next to this script ARE the source of truth for the copy.
// Spain's 112 posts existed only in its database, and when the box was rebuilt
// on 2026-08-07 they had to be recovered out of a chat transcript. Keeping the
// text in the repo means a lost database costs a re-insert, not an excavation.
//
// Usage:
//   node scripts/blog-it/insert.mjs batch-01.mjs [--dry-run]
//
// Run it from a deployed colour directory on the Italian box, so `pg` resolves
// and /var/www/photoitaly/.env is readable:
//   cd /var/www/photoitaly-blue && node scripts/blog-it/insert.mjs batch-01.mjs
...
if (!file) {
  console.error("usage: node scripts/blog-it/insert.mjs <batch-file.mjs> [--dry-run]");
  process.exit(1);
}
...
// Guard rails. Every one of these has bitten a market already: a slug that does
// not exist renders a link to nothing, and a locale the market does not ship
// puts the post on a URL that 404s.
const LOCALES = new Set(["en", "it", "de", "fr", "es"]);
...
const LOCATION_SLUGS = new Set([
  "rome", "florence", "siena", "val-dorcia", "pisa", "lucca", "venice", "verona",
  "lake-garda", "milan", "lake-como", "naples", "amalfi-coast", "positano",
  "sorrento", "capri", "ravello", "taormina", "palermo", "syracuse",
  "cinque-terre", "portofino", "lecce", "alberobello",
]);
...
for (const p of POSTS) {
  const r = await client.query(
    `INSERT INTO blog_posts
     ...
     VALUES ($1,$2,$3,$4,$5,$6,$7,'Photo Italy',TRUE,COALESCE($10::timestamptz, NOW()),$8,$9)
