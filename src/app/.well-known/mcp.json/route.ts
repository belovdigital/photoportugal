...
import { MARKETS, bookableDestinations, enabledMarkets } from "@/lib/phototravel/catalogue";
 ...
 * MCP server card.
 *
 * We built the PhotoTravel endpoint and then left it undiscoverable: an
 * agent had to be told the URL by a human. This is the file agent-readiness
 * checkers look for, and the one an MCP client reads to find out that a
 * booking tool exists here at all.
 *
...
export function GET() {
  ...
  // Spain is switched off in the PhotoTravel catalogue until it has a
  // roster, and a card on photospain.co offering Portuguese destinations
  // would be worse than no card. Flipping that flag publishes this too.
  ...
  return NextResponse.json(
    {
      ...
      endpoint: `${country.baseUrl}/api/mcp`,
      ...
      documentation: `${country.baseUrl}/llms.txt`,
