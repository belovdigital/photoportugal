// Insert Photo Portugal blog posts from a batch file into the Portuguese database.
...
// after the 2026-08-07 Spanish box wipe).
//
// Usage, from a deployed colour directory on the Portuguese box:
//   node scripts/blog-pt/insert.mjs batch-01.mjs [--dry-run]
...
if (!file) {
  console.error("usage: node scripts/blog-pt/insert.mjs <batch-file.mjs> [--dry-run]");
  process.exit(1);
}
...
const LOCATION_SLUGS = new Set([
  "lisbon", "sintra", "cascais", "caparica", "ericeira", "almada", "setubal",
  "comporta", "guimaraes", "porto", "braga", "douro-valley", "aveiro", "tomar",
  "peniche", "leiria", "coimbra", "obidos", "nazare", "evora", "algarve",
  "lagos", "tavira", "portimao", "albufeira", "faro", "vilamoura", "sesimbra",
  "arrabida", "funchal", "ponta-delgada", "madeira", "azores",
]);
...
for (const p of POSTS) {
  ...
  for (const word of ["Photo Spain", "Photo Italy", "photospain.co", "photoitaly.co"]) {
...
for (const p of POSTS) {
  const r = await client.query(
    `INSERT INTO blog_posts
     ...
     VALUES ($1,$2,$3,$4,$5,$6,$7,'Photo Portugal',TRUE,COALESCE($10::timestamptz, NOW()),$8,$9)
