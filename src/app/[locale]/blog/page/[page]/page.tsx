...
export default async function BlogPaginatedPage({
  ...
  if (!currentPage || currentPage < 1) notFound();
      ...
      <section className="border-b border-warm-200 bg-warm-50">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            <Link href="/blog" className="px-3 py-1.5 rounded-full text-xs font-medium bg-primary-500 text-white shadow-sm">
              {tCat("all")}
            </Link>
            {["locations", "pricing", "elopements", "weddings", "couples", "family", "planning", "proposals", "solo", "comparisons"].map((slug) => (
              <Link key={slug} href={`/blog/category/${slug}`} className="px-3 py-1.5 rounded-full text-xs font-medium transition bg-warm-100 text-gray-500 hover:bg-warm-200">
                {tCat(slug)}
              </Link>
            ))}
          </div>
              ...
              {posts.map((post) => (
                <article key={post.id} className="group overflow-hidden rounded-xl border border-warm-200 bg-white transition hover:shadow-lg">
                  <Link href={`/blog/${post.slug}`}>
                    {post.cover_image_url ? (
                      <div className="aspect-[16/9] overflow-hidden">
                        <OptimizedImage src={post.cover_image_url} alt={post.title} width={400} className="h-full w-full transition group-hover:scale-105" />
                      </div>
                    ) : (
                  ...
                  <div className="p-5">
                    ...
                    <Link href={`/blog/${post.slug}`}>
                      <h2 className="mt-2 font-display text-xl font-bold text-gray-900 transition group-hover:text-primary-600">{post.title}</h2>
                    </Link>
                    {post.excerpt && <p className="mt-2 line-clamp-3 text-sm text-gray-500">{post.excerpt}</p>}
                    <Link href={`/blog/${post.slug}`} className="mt-4 inline-flex items-center text-sm font-semibold text-primary-600 transition hover:text-primary-700">
                      {t("readMore")}
                ...
                <div className="hidden sm:flex items-center gap-1">
                  {getPageNumbers().map((page, idx) =>
                    page === "ellipsis" ? (
                      ...
                      <span key={page} className="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-primary-600 text-sm font-semibold text-white">{page}</span>
                    ) : (
                      <Link key={page} href={page === 1 ? "/blog" : `/blog/page/${page}`} className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-warm-200 bg-white text-sm font-medium text-gray-700 transition hover:bg-warm-50 hover:border-primary-200">{page}</Link>
                    )
                  )}
                </div>

                <span className="sm:hidden px-3 py-2 text-sm text-gray-500">Page {currentPage} of {totalPages}</span>

                {currentPage < totalPages ? (
                  <Link href={`/blog/page/${currentPage + 1}`} className="inline-flex items-center gap-1 rounded-lg border border-warm-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-warm-50 hover:border-primary-200">
                    Next
