          ...
          {isFeatured ? (
            <div className="mt-4">
              <div className="flex items-center gap-2 rounded-lg bg-yellow-50 p-3">
                ...
                <span className="text-sm font-medium text-yellow-700">Active — Featured placement</span>
              </div>
              <p className="mt-2 text-xs text-gray-400">Renews monthly. Cancel anytime from the billing portal.</p>
              <button
                onClick={handlePortal}
                disabled={!!loading}
                className="mt-2 w-full rounded-lg border border-red-200 px-4 py-2 text-sm font-medium text-red-500 hover:bg-red-50 disabled:opacity-50"
              >
                {loading === "portal" ? "Loading..." : "Cancel Featured Subscription"}
              </button>
            </div>
          ) : (
            <>
