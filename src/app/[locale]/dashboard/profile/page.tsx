...
export default async function ProfilePage() {
  ...
  const profile = await queryOne<{
    id: string; slug: string; display_name: string; first_name: string; last_name: string; tagline: string | null;
    ...
    `SELECT pp.id, pp.slug, pp.display_name, COALESCE(u.first_name, '') as first_name, COALESCE(u.last_name, '') as last_name,
  ...
  }>("SELECT id, name, description, duration_minutes, num_photos, price, is_popular, COALESCE(delivery_days, 7) as delivery_days FROM packages WHERE photographer_id = $1 ORDER BY sort_order, price", [profile.id]);
