...
export function AddOnsSection({ isVerified, isFeatured, phoneVerified: initialPhoneVerified, phoneNumber: initialPhone }: AddOnsSectionProps) {
  ...
  async function handleSendCode() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.error || t("failedSendCode"));
      }
    } catch {
      setError(t("failedSendCode"));
    }
    setLoading("");
  }
  ...
  async function handleBuyVerified() {
    ...
    try {
      const res = await fetch("/api/stripe/verified", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleGetFeatured() {
    setLoading("featured");
    try {
      const res = await fetch("/api/stripe/featured", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ locale }),
      });
        ...
        <div className="rounded-xl border border-warm-200 p-5">
          ...
          {isVerified ? (
            ...
            <div className="mt-4">
              ...
              {initialPhone && (
                <p className="mt-2 text-xs text-gray-400">{t("phoneLabel", { phone: initialPhone })}</p>
              )}
