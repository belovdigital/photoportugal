              ...
              {photographers.map((p) => (
                <tr key={p.id}>
                  <td className="px-4 py-3 font-medium text-gray-900">
                    <Link href={`/photographers/${p.slug}`} className="hover:text-primary-600">
                      {p.display_name}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-gray-500">{p.email}</td>
                  <td className="px-4 py-3">
                    <span className="rounded-full bg-warm-100 px-2 py-0.5 text-xs font-semibold uppercase text-warm-700">
                      {p.plan}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-gray-700">{p.rating || "—"}</td>
                  <td className="px-4 py-3 text-gray-700">{p.review_count}</td>
                  <td className="px-4 py-3">
                    <AdminToggle id={p.id} field="is_verified" value={p.is_verified} />
                  </td>
                  <td className="px-4 py-3">
                    <AdminToggle id={p.id} field="is_featured" value={p.is_featured} />
                  </td>
                  <td className="px-4 py-3">
                    <AdminPlanSelect id={p.id} currentPlan={p.plan} />
                  </td>
                </tr>
              ))}
            </tbody>
              ...
              {bookings.map((b) => (
                <tr key={b.id}>
                  ...
                  <td className="px-4 py-3 text-gray-500">
                    {b.shoot_date ? new Date(b.shoot_date).toLocaleDateString() : "—"}
                  </td>
                ...
                <tr key={u.id}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${
                      ...
                      u.role === "photographer" ? "bg-primary-100 text-primary-700" :
...
// Wrapper components
function AdminToggle({ id, field, value }: { id: string; field: string; value: boolean }) {
  return <AdminToggleClient id={id} field={field} value={value} />;
}

function AdminPlanSelect({ id, currentPlan }: { id: string; currentPlan: string }) {
  return <AdminPlanSelectClient id={id} currentPlan={currentPlan} />;
}
