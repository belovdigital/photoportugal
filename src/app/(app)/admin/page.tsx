...
import { payoutReadySql } from "@/lib/payout";
...
export default async function AdminPage() {
  ...
  const [
    ...
    queryOne<{ count: string }>(`SELECT COUNT(*) as count FROM photographer_profiles pp JOIN users u ON u.id = pp.user_id WHERE pp.is_approved = FALSE AND COALESCE(pp.is_test, FALSE) = FALSE AND COALESCE(u.email_verified, FALSE) = TRUE AND COALESCE(u.is_banned, FALSE) = FALSE AND pp.approval_requested_at IS NOT NULL AND pp.stripe_hidden_at IS NULL AND (pp.revision_status IS NULL OR pp.revision_status = 'submitted') AND u.avatar_url IS NOT NULL AND pp.cover_url IS NOT NULL AND pp.bio IS NOT NULL AND LENGTH(pp.bio) > 10 AND (SELECT COUNT(*) FROM portfolio_items WHERE photographer_id = pp.id) >= ${MIN_PORTFOLIO_PHOTOS} AND (SELECT COUNT(*) FROM packages WHERE photographer_id = pp.id AND custom_for_user_id IS NULL) >= 1 AND (SELECT COUNT(*) FROM photographer_locations WHERE photographer_id = pp.id) >= 1 AND u.phone IS NOT NULL`),
    ...
    queryOne<{ gross: string; fee: string; payout: string; sold: string; gifted: string }>(
      // The fee is per PAYMENT and these rows are per PHOTO, so it is joined to
      // the distinct payments of this set rather than to the rows — joining it
      // to the rows would charge a six-photo basket six times over.
      `SELECT COALESCE(SUM(amount_cents), 0) / 100.0 AS gross,
              (COALESCE(SUM(platform_fee_cents), 0) - COALESCE((
                 SELECT SUM(f.fee_cents) FROM stripe_payment_fees f
                  WHERE f.payment_intent_id IN (
                    SELECT DISTINCT stripe_payment_intent_id FROM delivery_extra_purchases
                     WHERE status = 'paid' AND stripe_payment_intent_id IS NOT NULL)), 0)) / 100.0 AS fee,
                  ...
                  WHERE f.payment_intent_id IN (
                    SELECT DISTINCT stripe_payment_intent_id FROM delivery_extra_purchases
                     WHERE status = 'paid' AND stripe_payment_intent_id IS NOT NULL
                       AND paid_at >= date_trunc('month', CURRENT_DATE))), 0)) / 100.0 AS fee
  ...
  const photographers = await query<{
    ...
    `SELECT pp.id, u.id as user_id, u.name as display_name, pp.slug, pp.plan, pp.rating, pp.review_count,
            ...
            pp.revision_status, pp.approval_requested_at, pp.stripe_deadline_at, pp.stripe_hidden_at,
  ...
  const stats = {
    ...
    stripeFeesPaid: parseFloat(
      (await queryOne<{ total: string }>(
        `SELECT COALESCE(SUM(f.fee_cents), 0) AS total
           FROM stripe_payment_fees f
          WHERE f.payment_intent_id IN (
                SELECT stripe_payment_intent_id FROM bookings WHERE payment_status = 'paid'
                 UNION SELECT stripe_payment_intent_id FROM delivery_extra_purchases WHERE status = 'paid'
                 UNION SELECT stripe_payment_intent_id FROM tips WHERE status = 'paid')`
      ).catch(() => null))?.total || "0"
    ) / 100,
